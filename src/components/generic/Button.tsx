...
const tailwindClass =
  'text-white bg-gradient-to-r from-cyan-500 to-blue-500 hover:bg-gradient-to-bl focus:ring-1 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800 font-medium rounded text-sm px-5 py-2 text-center mr-2 mb-2';
...
export const Button: React.FC<React.ComponentProps<'button'> & ButtonProps> = (props) => {
  ...
  if (variant === 'secondary') {
    const innerButtonClass = classNames(
      props.className,
      'relative px-5 py-2 transition-all ease-in duration-75 bg-white dark:bg-gray-900 rounded group-hover:bg-opacity-0'
    );
